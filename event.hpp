...
private:
	//Note = 0 | tempo = 1 | endTrack = 2
	enum EventType { note, tempo, endTrack } eventType;

	union Events {
		NoteEvent noteEvent_; //uses underscore to specify class variable
		TempoEvent tempoEvent_;
		EndTrackEvent endTrack_;
		Events() { EndTrackEvent(); }
		~Events() {};
	} event;

	intmax_t midi_ticktime_;
};
